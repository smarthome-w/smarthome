...
String calculateMessageName(int sensorType, int valueType, String overrideMultisensorName)
{
  ...
  else if (sensorType == SENSOR_BME280)
  {
    if (valueType == SENSOR_VALUE_TEMPERATURE)
    {
      messageName = GLOBAL_MQTT_MESSAGE_PREFIX + "/" + GLOBAL_MQTT_MULTISENSOR_NAME + "_Indoor_Temperature_Sensor1";
    }
    else if (valueType == SENSOR_VALUE_HUMIDITY)
    {
      messageName = GLOBAL_MQTT_MESSAGE_PREFIX + "/" + GLOBAL_MQTT_MULTISENSOR_NAME + "_Humidity_Sensor1";
    }
    else if (valueType == SENSOR_VALUE_PRESSURE)
    {
      messageName = GLOBAL_MQTT_MESSAGE_PREFIX + "/" + GLOBAL_MQTT_MULTISENSOR_NAME + "_Pressure1";
    }
  }
